...
CREATE TABLE IF NOT EXISTS banquet_orders (
  ...
  guest_count  INTEGER,
  budget       REAL,
  service_fee  REAL,                    -- 服务费（按价格带匹配带入，可手改）
...
CREATE TABLE IF NOT EXISTS price_bands (
  ...
  service_fee REAL NOT NULL DEFAULT 0,  -- 整单服务费（一口价）
...
CREATE TABLE IF NOT EXISTS materials (
  ...
  unit        TEXT,
  unit_price  REAL,
  stock       REAL NOT NULL DEFAULT 0,   -- 当前库存数量
  notes       TEXT,
  created_at  TEXT DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS order_materials (
  ...
  planned_qty REAL NOT NULL DEFAULT 0,   -- 需要准备的数量（下单时从库存扣减）
  actual_qty  REAL,                       -- 宴席后管理员的清点结果
  unit_price  REAL,                       -- 准备时的物料单价快照
  created_at  TEXT DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS settlements (
  ...
  total_amount     REAL NOT NULL DEFAULT 0,   -- 营收口径 = 实际收款（派生）
  actual_cost      REAL NOT NULL DEFAULT 0,   -- 成本口径（总损耗）= 额外损耗 + 物料损耗费（派生）
  received_amount  REAL NOT NULL DEFAULT 0,   -- 实际收款金额
  extra_loss       REAL NOT NULL DEFAULT 0,   -- 额外损耗（可选，人工估损等）
  payment_method   TEXT,
  notes            TEXT,
...
-- 一些演示数据
INSERT OR IGNORE INTO villages (name) VALUES ('李家村'), ('王家村'), ('赵庄');

INSERT OR IGNORE INTO customers (name, phone, village_id, address, notes) VALUES
...
INSERT OR IGNORE INTO materials (name, category, unit, unit_price, stock, notes) VALUES
